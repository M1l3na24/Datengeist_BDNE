'''
predictor.py — Módulo de predicción de ventas para Datengeist

Responde las preguntas de análisis que requieren ML:
  4b. ¿Cuánto se debe producir hoy para maximizar la frescura?
  7a. ¿Cuál es la proyección de ventas semanales y mensuales de los próximos 3 meses?

Flujo:
  1. Lee kpi_ventas_temporales (histórico mensual) desde MongoDB
  2. Entrena un modelo de regresión lineal con features de estacionalidad y lag
  3. Predice los próximos 3 meses de ingresos y tickets
  4. Calcula recomendación de producción diaria por producto/sabor
...
if len(df) < 12:
    print('❌ Se necesitan al menos 12 meses de histórico para predecir. Ejecuta etl.py primero.')
    exit(1)
...
# Features: índice de tiempo, seno/coseno de la estacionalidad anual, lag 1 y lag 12
...
# ── 3. Predecir los próximos 3 meses ─────────────────────────────────────────
...
# ── 4. Recomendación de producción diaria (próximo mes) ──────────────────────
# Calcula las unidades diarias sugeridas por producto basándose en:
#   mix histórico de cada producto en el total de unidades vendidas
#   × tickets diarios estimados del próximo mes
print('\n⏳ Calculando recomendaciones de producción...')

raw_prod = list(mongo['kpi_productos_sabores'].find({}, {'_id': 0}))
if not raw_prod:
    print('❌ kpi_productos_sabores vacío. Ejecuta etl.py primero.')
    exit(1)
...
# Promedio histórico de unidades por ticket
avg_u_ticket = float(
